...
def line_fitnmerge(cfg, imagecols, depths, neighbors=None, ranges=None):
    ...
    if neighbors is None:
        _, neighbors, ranges = _runners.compute_sfminfos(cfg, imagecols) # 如果一開始沒有鄰居的話，利用COLMAP point triangulation(SFM -> compute neighbor)計算鄰居給他
    else:
    ...
    ##########################################################
    all_2d_segs, _ = _runners.compute_2d_segs(cfg, imagecols, compute_descinfo=cfg["line2d"]["compute_descinfo"]) # 產出線段與線段的描述子，描述子資料存在資料夾中
    ...
    ##########################################################
    # 對合併後的3D線段進行Bundle Adjustment幾何優化
