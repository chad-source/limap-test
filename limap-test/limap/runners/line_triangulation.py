...
def line_triangulation(cfg, imagecols, neighbors=None, ranges=None):
    ...
    #     if img_id == 0:
    #         neighbors[img_id] = [1]
    #     elif img_id == len(imagecols) - 1:
    #         neighbors[img_id] = [img_id - 2]
    #     else:
    #         neighbors[img_id] = [img_id - 1, img_id + 1]
    ...
    all_2d_segs, descinfo_folder = _runners.compute_2d_segs(cfg, imagecols, compute_descinfo=compute_descinfo) # 產出線段與線段的描述子，描述子資料存在資料夾中

    ##########################################################
    # [C] get line matches
    ##########################################################
    if not cfg["triangulation"]["use_exhaustive_matcher"]:
        matches_dir = _runners.compute_matches(cfg, descinfo_folder, imagecols.get_img_ids(), neighbors) # 將影像中的線段與鄰居線段連接(2D)
