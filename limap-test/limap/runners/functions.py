...
def compute_2d_segs(cfg, imagecols, compute_descinfo=True):
    ...
    if not cfg["load_det"]:
        if compute_descinfo and cfg["line2d"]["detector"]["method"] == cfg["line2d"]["extractor"]["method"] and (not cfg["line2d"]["do_merge_lines"]):
            ...
            all_2d_segs = detector.detect_all_images(folder_save, imagecols, skip_exists=se_det) # 做line detection，最後產出線段
            if compute_descinfo:
                extractor = limap.line2d.get_extractor(cfg["line2d"]["extractor"], weight_path=weight_path) # 提取線段描述子
